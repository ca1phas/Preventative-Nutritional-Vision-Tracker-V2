...
const MOCK_INSIGHTS = [
    { type: 'success', icon: 'fa-check-circle', title: 'Great Progress!', message: 'You\'ve maintained healthy calorie intake for 5 consecutive days.' },
    { type: 'warning', icon: 'fa-exclamation-triangle', title: 'High Carbohydrate Intake', message: 'Your carb intake averaged 220g/day this week, which is 15% above recommended levels for pre-diabetic patients.' },
    { type: 'info', icon: 'fa-info-circle', title: 'Eating Pattern Analysis', message: 'You tend to consume 60% of daily calories after 6 PM. Consider earlier meal times for better glucose management.' },
    { type: 'danger', icon: 'fa-heartbeat', title: 'Risk Indicator', message: 'Blood sugar spike risk detected on March 7. High sugar intake at dinner (estimated 85g).' }
];
...
async function fetchUserProfile() {
    ...
    try {
        ...
        if (data) {
            ...
            if (data.gender.toLowerCase() === 'male') bmr += 5;
            else bmr -= 161;
...
function updateStats(periodData) {
    ...
    updateOverallBannerStatus(Math.round(totalMain.carbs / divisor));
...
function updateOverallBannerStatus(avgCarbs) {
